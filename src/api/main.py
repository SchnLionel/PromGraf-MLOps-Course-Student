...
async def evaluate_model(items: List[EvaluationItem]):
    ...
    try:
        ...
        # Note: classifier.model.config.id2label might not be available directly on the pipeline object depending on the version
        # but for this specific model it should work or we extract it safely.
        model_config = getattr(classifier, 'model', None)
        if model_config and hasattr(model_config, 'config'):
            model_categories = [val.lower() for val in model_config.config.id2label.values()]
        else:
            model_categories = []
            
        all_categories = sorted(list(set([item.true_label.lower() for item in items] + model_categories)))
